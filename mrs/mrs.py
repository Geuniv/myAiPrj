...
# 데이터 불러오기
# 2개의 csv 파일을 사용하므로, 같은 필드인 id를 이용하여 병합
movies = pd.read_csv("C:/myAiPrj/data/movie/tmdb_5000_movies.csv")
credits = pd.read_csv("C:/myAiPrj/data/movie/tmdb_5000_credits.csv")
...
# 데이터 범위 지정 - 최소 투표 1000 이상
# vote_count가 1000 이상인 영화만 선택
m = 1000
data = data[data['vote_count'] >= m]

# 다른 예시 - 최소조건은 전체 투표의 90% 이상 ( 하위 10% 버림 )
...
print("Top 10 Movies by Weighted popularity")
...
dot_user_moive = Dot(axes=1, name='dot_user_movie')([user_vec, movie_vec])

output = Dense(1, activation='linear', name='output')(dot_user_moive)
...
ax2.set_xticklabels(top_10_movies_score['title'], rotation=45)
...
for i, (popularity, score) in enumerate(zip(top_10_movies_popularity['popularity'], top_10_movies_score['score'])):
